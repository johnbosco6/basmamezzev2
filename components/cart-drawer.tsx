...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div
                className={`fixed top-0 right-0 h-full w-full max-w-md z-[70] flex flex-col bg-gradient-to-b from-[#2B2B2B] to-[#1a1a1a] shadow-2xl border-l border-[#BA9D76]/30 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                {/* Header */}
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className="w-7 h-7 rounded-lg bg-white/10 hover:bg-[#BA9D76]/30 text-white flex items-center justify-center transition-colors duration-200"
                                        >
                                            <Minus className="h-3 w-3" />
                                        </button>
                                        <span className={`text-white font-semibold text-sm w-4 text-center ${archivo.className}`}>
                                            {item.quantity}
                                        </span>
                                        <button
                                            onClick={() => updateQty(item.id, item.quantity + 1)}
                                        ...
                                        <button
                                            ...
                                            className="ml-auto w-7 h-7 rounded-lg bg-red-500/10 hover:bg-red-500/30 text-red-400 flex items-center justify-center transition-colors duration-200"
                                            aria-label="Usuń"
                                        >
